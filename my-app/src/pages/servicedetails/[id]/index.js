...
import serimg from "../../../../public/serviceside.jpg";
...
const Singleservices = () => {
  ...
  useEffect(() => {
    const fetchCarousel = async () => {
      ...
      try {
        let response = await getSingleServices(id);
        let Carousel1 = response.data;
        setData(Carousel1);
        ...
        <Container className={`${Styles.container} grid grid-cols-5 gap-4`}>
          ...
          {data ? (
            <div className={`col-span-5 md:col-span-4 ${Styles.about_main}`}>
              <h1 className={`text-3xl font-semibold ${Styles.mainTitle}`} style={{ color: "#102c44" }}>
                {data.service}
              </h1>

              {/* Carousel Wrapper with Proper Sizing */}
              <div className="w-full max-w-full overflow-hidden">
                ...
                >
                  {mainCarousel?.map((imageSrc, index) => {
              ...
              {/* Show FormPopup on service details page */}
              <FormPopup show={true} />

              <div className="mt-3 text-xl break-words" dangerouslySetInnerHTML={{ __html: data.descr }} />
            </div>
          ) : (
