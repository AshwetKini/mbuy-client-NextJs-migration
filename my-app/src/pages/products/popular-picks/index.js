...
import { getRecommendedProduct, getProduct, getVariant, getSlugName } from "@/apis/api";
import Layout from "@/Layouts/Layout";
import { FaShippingFast, FaFilter, FaTimes } from "react-icons/fa";
import { useRouter } from "next/router";

const PopularPicks = ({ initialProducts }) => {
  ...
  useEffect(() => {
    if (!initialProducts) {
      const fetchRecommendedData = async () => {
        try {
          setLoading(true);
          const recommendedResponse = await getRecommendedProduct();
          const combinedRec = [
            ...(recommendedResponse.products || []),
            ...(recommendedResponse.allVariations || []),
          ];
          setProducts(combinedRec);
        } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Popular Picks - Top Products Chosen by Our Customers</title>
        <meta
          name="description"
          content="Discover our most popular products loved by customers. Shop trending items at great prices with fast shipping."
        />
      </Head>
      ...
      <div className="w-full bg-white flex flex-col md:flex-row min-h-screen relative">
        ...
        {isMobileFilterOpen && (
          <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-[999]">
...
export async function getStaticProps() {
  try {
    ...
    console.error("Error fetching Popular Picks data:", error);
    return {
      props: {
        initialProducts: [],
      },
