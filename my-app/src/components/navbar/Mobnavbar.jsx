...
const Trailnavbar = () => {
  ...
  return (
    <div className="bg-gray-900 flex justify-between md:hidden items-center h-[55px] trailnavmob relative ">
      <Menu
        as="div"
      ...
      >
        ...
        <Menu.Items className="absolute top-[100%] p-3 left-0 mt-2 z-50 w-[150px] origin-top-left rounded-md bg-gray-900 shadow-lg ring-1 ring-black/5 focus:outline-none">
          {categories.categoriesData?.map((item, index) => (
            <Menu.Item key={index}>
              <Link
                href={`/allproducts/${(item.title ?? '').replace(/\s+/g, '-')}`}
                className={
                  "text-white group flex w-full items-center rounded-md px-2 py-2 text-sm"
                }
              >
                {item.title.charAt(0).toUpperCase() + item.title.slice(1).replace(/-/g, ' ').replace(/\s(\w)/g, (match) => match.toUpperCase())}
              </Link>
            </Menu.Item>
          ))}
        </Menu.Items>
      ...
      >
        ...
        <Menu.Items className="absolute top-[100%] p-3 right-0 mt-2 z-50 w-56 origin-top-right rounded-md bg-gray-900 shadow-lg ring-1 ring-black/5 focus:outline-none">
          {/* <Menu.Item>
            <Link
              href="/allproducts/categories/all"
              className={
                "text-white group flex w-full items-center rounded-md px-2 py-2 text-sm"
              }
            >
