...
const ProductDetails = (props) => {
    ...
    try {
      ...
      try {
        ...
        if (slugName?.data && slugName.data.length > 0 && (slugName.data[0]?.old_url || slugName.data[0]?.url_structure)) {
          ...
          // Determine if this is a variant or a main product
          ...
          navigation.push(`/${path}/${urlPath}/${item._id}`);
        } else {
          ...
          navigation.push(`/${path}/${fallbackUrl}/${item._id}`);
        }
      } catch (apiError) {
      ...
      if (item && item._id) {
        ...
        // Determine if this is a variant or a main product as a last resort
        const isVariant = item.mainProductId !== undefined && item.mainProductId !== null && item.mainProductId !== "";
        const path = isVariant ? "variance" : "products";
  ...
  // Add loading state for pincode check
  ...
  const processLogisticResponse = (logisticData) => {
    ...
    logisticData.pricesresult.forEach((price, index) => {
      ...
      console.log(`Option ${index}: Product: ${productPrice.toFixed(2)}, Shipping: ${shippingCost.toFixed(2)}, Total: ${totalCost.toFixed(2)}`);
    ...
    // Set the product price from the selected option - ensure it's rounded to 2 decimal places
    const selectedPrice = Number(logisticData.pricesresult[lowestPriceIndex]) || 0;
    setVendorPriceAtPincode(Math.round(selectedPrice * 100) / 100);

    console.log(`Selected option ${lowestPriceIndex} with product price ${selectedPrice.toFixed(2)} and total cost ${lowestTotalCost.toFixed(2)}`);
    console.log(`Selected vendor/warehouse:`, selectedVendorWarehouse);
    
    // Set pincode checked to true to enable the UI
    ...
    const getProductDetails = async () => {
      try {
        if (props.productData != null) {
          console.log("Product data received:", props.productData);
          
          // Check if we have the essential product data
          if (!props.productData._id) {
            console.error("Product data missing ID:", props.productData);
          }
          ...
          if (props.productData.discountprice2B) {
            ...
            console.warn("Product missing discountprice2B:", props.productData);
          }
          ...
          if (cart.cart.length > 0) {
            const check = cart.cart.find(
              (cartItem) => 
                (cartItem.item && cartItem.item._id === props.productData._id) || 
                (cartItem.item && cartItem.item.productDetails && cartItem.item.productDetails._id === props.productData._id)
            );
          ...
          console.error("No product data received in props");
        }
        
        // Process frequently bought together products
        if (props.productData?.vari && props.productData.vari.length > 0) {
          setIsFreqLoading(true); // Start loading
          console.log("Processing frequently bought together products with IDs:", props.productData.vari);
          
          // Initialize an array to collect all fetched products
          let fetchedProducts = [];
          
          // Process each ID in the vari array
          for (const itemId of props.productData.vari) {
            if (!itemId) continue; // Skip if ID is null or undefined
            
            try {
              // Try to fetch as a product first
              let productResponse = await productDetailsWithID(itemId);
              
              if (productResponse?.status === 200 && productResponse?.data) {
                console.log(`Found item ${itemId} as a product`);
                // Process the product data
                const productData = Array.isArray(productResponse.data) 
                  ? productResponse.data[0] 
                  : productResponse.data;
                
                if (productData && productData._id && productData._id !== props.productData._id) {
                  fetchedProducts.push(productData);
                }
              } else {
                // If not found as a product, try as a variant
                console.log(`Trying item ${itemId} as a variant`);
                let variantResponse = await getVariant(itemId);
                
                if (variantResponse?.status === 200 && variantResponse?.data) {
                  console.log(`Found item ${itemId} as a variant`);
                  // Process the variant data
                  const variantData = Array.isArray(variantResponse.data) 
                    ? variantResponse.data[0] 
                    : variantResponse.data;
                  
                  if (variantData && variantData._id && variantData._id !== props.productData._id) {
                    fetchedProducts.push(variantData);
                  }
                }
              }
            } catch (error) {
          ...
          if (fetchedProducts.length > 0) {
            console.log(`Successfully fetched ${fetchedProducts.length} related products`);
            setFreqB(fetchedProducts);
          }
          setIsFreqLoading(false); // End loading regardless of result
        } else {
          setIsFreqLoading(false); // No products to load
        }
      } catch (error) {
        console.error("Error in getProductDetails:", error);
        setIsFreqLoading(false); // End loading on error
      }
    };
    getProductDetails();
  ...
  const radiochangehandler = (min, price) => {
    ...
    if (productDetails.norate) {
      // If norate is true, always use discountprice2B regardless of pincode check
      dispatchQuantity({
        type: "PRICE",
      ...
      if (pincodeChecked) {
        const totalPrice = Number(price) + Number(vendorPriceAtPincode || 0);
        dispatchQuantity({
          type: "PRICE",
          price: totalPrice,
        });
  ...
  useEffect(() => {
    // Only sync the userinput state with quantity if quantity is not empty
    // This prevents the field from being reset during typing
  ...
  const incrementquantityhandler = () => {
    // Handle case when quantity might be empty
    const currentQuantity = quantityState.quantity === '' 
      ? parseInt(productDetails?.minord11A) || 1 
      : quantityState.quantity;
      
    // Check if incrementing would exceed maxord11B
    if (currentQuantity + 1 > productDetails.maxord11B) {
  ...
  const decrementquantityhandler = () => {
    ...
    // Only decrement if not already at minimum
    if (currentQuantity > productDetails.minord11A) {
  ...
  const setthequantity = () => {
    ...
    if (Number(userinput) > Number(productDetails.maxord11B)) {
      ...
      dispatchQuantity({ type: "USER_INPUT", value: productDetails.maxord11B });
      return;
    }
    ...
    // Convert userinput to a number for comparison
    const numericInput = Number(userinput);
    
    // Only set to minord11A if the userinput is less than minord11A
    if (numericInput < Number(productDetails.minord11A)) {
      setUserInput(productDetails.minord11A);
      dispatchQuantity({ type: "USER_INPUT", value: productDetails.minord11A });
    } else {
      // Keep the user's input if it's valid (between min and max)
      dispatchQuantity({ type: "USER_INPUT", value: numericInput });
      setUserInput(numericInput);
    }
  };
  ...
  const [filteredVariants, setFilteredVariants] = useState(
    props.productData?.variations || []
  );

  const variantOptions = {
    vari1: [
      ...new Set(
        props.productData?.variations?.map((v) => v.vari11).filter(Boolean)
      ),
    ],
    vari2: [
      ...new Set(
        props.productData?.variations?.map((v) => v.vari12).filter(Boolean)
      ),
    ],
    vari3: [
      ...new Set(
        props.productData?.variations?.map((v) => v.vari13).filter(Boolean)
      ),
    ],
    vari4: [
      ...new Set(
        props.productData?.variations?.map((v) => v.vari14).filter(Boolean)
      ),
    ],
  };
  ...
  useEffect(() => {
    if (productDetails && productDetails?._id) {
      ...
      if (props.productData) {
        filtered.push(props.productData);
      }
      
      // Then add variations if they exist
      if (props.productData?.variations) {
        filtered = [...filtered, ...props.productData.variations];
      }

      // Only apply filtering if there are selected values
      if (Object.values(selectedValues).some(value => value)) {
        filtered = filtered.filter(variant => {
          return Object.entries(selectedValues).every(([key, value]) => {
            if (!value) return true;
            const mappedKey = key === "vari1" ? "vari11" : 
                            key === "vari2" ? "vari12" : 
                            key === "vari3" ? "vari13" : 
                            key === "vari4" ? "vari14" : null;
            return mappedKey && variant[mappedKey] === value;
          });
        });
      }
  ...
  }, [selectedValues, props.productData?.variations, props.productData, productDetails]);
...
// console.log(data,"button2");
  ...
  const deletefromWishlist = async () => {
    ...
    const data = {
      userId: getuserId,
      productId: productDetails.mainProductId || productDetails._id, // Ensure proper ID is used
      // varianceId: productDetails.mainProductId ? productDetails._id : undefined, // Only include varianceId if it's a variant
    };
  ...
  useEffect(() => {
    ...
    const getWishlistData = async (id) => {
      try {
        ...
        // console.log(response.data, "wishlist yahi hai ");

        if (response.status === 200 && productDetails) {
          // console.log(response, "andar aa rha hai");
          const isProductInWishlist = response.data?.whishList?.some(
            (item) =>
              item.productId === productDetails._id  // Ensure it matches the product
              // (productDetails.mainProductId || item.varianceId === productDetails._id) // Match variant only if it exists
          );
  ...
  const changetheproduct = async (item, index) => {
    try {
      ...
      if (slugName?.data && slugName.data.length > 0 && (slugName.data[0]?.old_url || slugName.data[0]?.url_structure)) {
        // Use old_url if available, otherwise url_structure
        const urlPath = slugName.data[0]?.old_url || slugName.data[0]?.url_structure;
        navigation.push(item.mainProductId ? 
...
useEffect(() => {
  ...
  const calculatePrices = (quantity, productDetails, isUserCorporate) => {
    ...
    if (productDetails.norate) {
      ...
      totalPrice = productDetails.discountprice2B * quantity;
    } else {
      // Otherwise follow original logic
      totalPrice = pincodeChecked ? (unitPrice * quantity) : (productDetails.discountprice2B * quantity);
    }
    ...
    if (productDetails.norate) {
      ...
      gstAmount = ((totalPrice + (pincodeChecked ? (vendorPriceAtPincode * quantity) : 0)) * gstPercentage) / 100;
    }
    ...
    if (productDetails.norate) {
      ...
      finalPrice = totalPrice + gstAmount + (pincodeChecked ? (vendorPriceAtPincode * quantity) : 0);
    }
  ...
  // Update the price calculation logic in the useMemo to handle empty string quantity
  const priceCalculation = useMemo(() => {
    // If quantity is empty string (during user input), use minimum quantity for calculation
    const calculationQuantity = quantityState.quantity === '' 
      ? parseInt(productDetails?.minord11A) || 1 
      : quantityState.quantity;
  ...
  useEffect(() => {
    ...
    const checkIfInCart = () => {
      if (productDetails && cart.cart.length > 0) {
        ...
        // Find item in cart
        const check = cart.cart.find(cartItem => 
                ...
                <div className="flex flex-col md:w-[70%] space-x-3">
                  <div className="w-full flex-col justify-center xl:flex-row flex">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {props.productData?.variations?.length > 0 && (
                        <div className="mt-4">
      ...
      Top Brands
                ...
                <div className="md:w-[30%] max-w-screen relative  ml-3 border-gray-300 border p-2 rounded-lg">
                  ...
                  <h2 className="text-gray-600">
                    ₹{" "}
                    {Math.ceil(priceCalculation.finalPrice)}{" "}
                    (Incl. all taxes)
                  </h2>
                    ...
                    {productDetails.partprice4A && (
                      <span className="px-1 bg-gray-300 text-sm items-center flex text-green-600 font-bold">
                        ...
                        {((pincodeChecked 
                            ...
                            : (Number(productDetails.discountprice2B)))
                  ...
                  >
                    ₹ 
                    {pincodeChecked
      ? `${Math.ceil(priceCalculation.totalPrice + vendorPriceAtPincode * quantityState.quantity)}/-`
  : `${Math.ceil(productDetails.discountprice2B * quantityState.quantity)}/-`}
<span className="ml-2 text-xs md:text-lg">
                    <s>₹ {Math.ceil(productDetails.price2A * quantityState.quantity)} /-</s>
                    </span>
                                  ...
                                  >
                                    <span className={!pincodeChecked ? 'blur-[3px]' : ''}>
                                      {productDetails.norate ? (
                                        <>₹{Math.ceil(Number(productDetails.discountprice2B))}</>
                                      ) : (
                                        <>₹{Math.ceil(Number(price) + Number(vendorPriceAtPincode || 0))}</>
                                      )}
                                    </span>
                                  </td>
                                ...
                                <div className="p-2">
                                  ...
                                  <div className="mt-1 flex items-center">
                                    <span className="text-red-600 font-bold">
                                      ₹{Math.ceil(product.discountprice2B) || "0.00"}
                                    </span>
                                    {product.price2A && (
                                      <span className="ml-1 text-xs text-gray-500 line-through">
                                            ₹{Math.ceil(product.price2A)}
                                      </span>
                                    )}
                                  </div>
                                </div>
...
// Skeleton for frequently bought together productsmai
const FrequentlyBoughtSkeleton = () => (
  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 md:gap-4">
    {[...Array(6)].map((_, index) => (
      <div key={index} className="py-1 flex justify-center">
        <div className="relative flex flex-col h-full w-full bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden animate-pulse">
          ...
          <div className="p-2">
            ...
            <div className="mt-2 flex items-center">
              ...
              <div className="ml-1 h-3 bg-gray-200 rounded w-1/5"></div>
            </div>
          </div>
        </div>
      </div>
