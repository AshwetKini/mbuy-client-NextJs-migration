...
import { getProduct, getProductVariant } from "../apis/api";
...
const SingleProductDetails = () => {
  ...
  const { productid, productName1 } = useParams();
  const [activevariant, setActiveVariant] = useState(null);
  const [mainproduct, setMainProduct] = useState(null);
  const [loading, setLoading] = useState(true);
  // const productName = productName1.replace(/-/g, " ");

  // useEffect(() => {
  //   setLoading(true);
  //   const getProductDetails = async () => {
  //     const productdetails = await getProduct(productid);
  //     console.log(productdetails.data?.data?.variations,"productdetails.data.data");
  //     setProductsDetails(productdetails.data?.data);
